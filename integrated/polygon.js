class Polygon {
     ...
     */
     draw(gl) {
        ...
        gl.drawArrays(gl.TRIANGLE_FAN, 0, this.vertexes.length / 2)
    }
    /**
    ...
    getClosestPoint(point) {
        ...
        for(let i = 0; i < this.getPointCount(); ++i) {
            if(closestPoint.length == 0) {
                closestPoint.push({
                    dist: calculateDistance(this.getPoint(i), point),
                    index: i
                })
            } else if(closestPoint.length == 1) {
                closestPoint.push({
                    dist: calculateDistance(this.getPoint(i), point),
                    index: i
                })
                if(closestPoint[0].dist > closestPoint[1].dist) {
                    const temp = closestPoint[0]
                    closestPoint[0] = closestPoint[1]
                    closestPoint[1] = temp
                }
            } else {
                const dist = calculateDistance(this.getPoint(i), point)
    
                if(dist < closestPoint[0].dist) {
                    closestPoint[1] = closestPoint[0]
                    closestPoint[0] = {
                        dist: dist,
                        index: i
                    }
                } else if(dist < closestPoint[1].dist) {
                    closestPoint[1] = {
                        dist: dist,
                        index: i
                    }
    ...
    addPoint(newPointLocation) {
        this.vertexes.push(newPointLocation.x)
        this.vertexes.push(newPointLocation.y)
        ...
        //     insertIdx = closestPoint[0].index
        // } else {
        //     insertIdx = closestPoint[1].index
        // }
        // const convertedIndex = insertIdx * 2
        // this.vertexes.insert(convertedIndex, newPointLocation.y)
        // this.vertexes.insert(convertedIndex, newPointLocation.x)

        // const convertedColorIdx = insertIdx * 3
